...
- [ ] Pilot tenant enablement evidence (allowlist + verification checklist above) — **sole allowlisted tenant:** `om_church_46` (slug + numeric `church_id` **46** confirmed 2026-07-19). **Partial evidence:** login, logout, expired-session, unauthorized, role enforcement, CSRF profile save, error logging confirmed 2026-07-19 on live `/portal2`; parish settings fix `f1aeb2d37` deployed (re-login to confirm live); nested `next=` post-login + rollback **still open** — `docs/AUTH-PILOT-CHECKLIST.md`, `pnpm validate:auth-pilot`
...
- [ ] Live authentication and church context work for pilot users — capability shipped (Wave B); **partial enablement evidence** for sole pilot `om_church_46`: login + church context + records + logout/session/unauthorized/role/CSRF confirmed 2026-07-19; parish settings re-verify after `f1aeb2d37`; nested `next=` post-login + rollback still open (`docs/AUTH-PILOT-CHECKLIST.md`)
